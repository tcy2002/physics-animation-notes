...
这里x(t)的写法不是指有一个新的函数，而是表达这里的x是与t相关的变量，要通过求解这个微分方程来得到，对于模拟来说，无法直接获得解析解，需要用到某些方法来估计。

考虑一个一般场景，空间中存在保守力场$U(x)$（比如重力场）和速度阻尼$D(v)$，粒子的初始位置为$x_0$，初始速度为$v_0$，质量为m，则其后续的运动满足二阶常微分方程：
...
隐式欧拉的形式：$x_{n+1}=x_n + hf(x_{n+1}, t)$，即把目标位置的变化率看作整个时间步长内的变化率，也是一阶精度，一般而言x_{n+1}很难直接得到，需要求解矩阵方程，比如[Cloth.md](./Cloth.md)中的隐式布料方法，或通过迭代法来求解，比如预报-校正法，从显式形式开始，得到的值逐步替换进去，最后得到比较准确的隐式结果：$\overline{x_{n+1}}=x_n + hf(x_n, t)\rightarrow x_{n+1}=x_n + hf(\overline{x_{n+1}}, t)$。
