...
这里采用Gauss-Seidel迭代来求解（参考[MathUtils.md](./MathUtils.md)），迭代公式为：
...
注意需要选取合适的阈值以使解算收敛。求出 $p^{(n+1)}$ 之后，再求出 $\nabla p^{(n+1)}$ ，最后使用式 (4) 来更新 $u^{(n+1)}$ . $u_{i+\frac{1}{2},j}^{(n)}$ 、 $u^{(n)}_{i-\frac{1}{2},j}$ 、 $u^{(n)}_{i,j+\frac{1}{2}}$ 、 $u^{(n)}_{i,j+\frac{1}{2}}$ 即前一步得到的 $u_B$ .

GS迭代本身不适合GPU并行，但由于网格的高度对称和稠密性，可采用一种“红黑”更新方式来达到与GS迭代相同的精度：把每个cell按照 $(i+j+k)%2$ 分成两组，先并行更新“红”，再并行更新“黑”，收敛速度约为普通Jacobi迭代的1.5-2倍。除此之外，还有PCG、MGPCG等面向流体求解的高精度求解器。

实际实现中，压力求解通常分为三步：
- 求解速度散度：式 (3)
- 压力求解（Jacobi、GS或PCG等）：式 (5)
- 速度投影：式 (4)
...
光滑粒子流体动力学（Smoothed Particle Hydrodynamics）使用一个叫做“光滑核函数”的概念来计算粒子携带的物理量（如密度，压力等），本质上是计算周围一定半径内邻居粒子的加权平均。

与网格法类似，SPH求解NS方程的三个步骤：

- 密度与压力：使用上述光滑核函数来计算密度，在最经典的WCSPH（弱可压缩SPH）中，直接将密度映射为压力： $p_i = B((\frac{\rho_i}{\rho_0})^\gamma - 1)$

- 压力梯度：由于流体总是从高压区流向低压区，粒子会受到一个压力梯度力。为了保证动量守恒（牛顿第三定律，作用力等于反作用力），SPH 通常采用对称化的压力公式： $\mathbf{f}_i^{pressure} = -\sum_j m_j \left( \frac{p_i}{\rho_i^2} + \frac{p_j}{\rho_j^2} \right) \nabla W(\mathbf{x}_i - \mathbf{x}_j, h)$

- 粘滞力：真实流体（如蜂蜜或水）在流动时有内部摩擦。粘滞力试图让相邻的粒子速度趋于一致： $\mathbf{f}_i^{viscosity} = \mu \sum_j m_j \frac{\mathbf{v}_j - \mathbf{v}_i}{\rho_j} \nabla^2 W(\mathbf{x}_i - \mathbf{x}_j, h)$

最终将重力、压力梯度力和粘滞力相加，用半隐式欧拉来更新速度和位置

除WCSPH外，还有一些其他方法试图获得隐式欧拉的精度：PCISPH（预测校正）、IISPH（暴力解矩阵方程，完全的隐式精度）

### PBF

基于位置的流体（Position Based Fluids）密度和压力与SPH的计算方式一致，但不通过积分方法来求位置更新，而是按照PBD的思路直接对位置做更新。
改进项：
- 压力补偿：粒子表面散开时，领居变少，导致计算出的密度偏低，粒子为了恢复密度会相互吸引，导致表面结块，可以引入一个微笑的人工排斥力，在极近的距离内推开粒子，保证表面粒子分布均匀
- 涡增强：PBD迭代会引入数值粘性，导致流体原本应该产生的漩涡、水花在几次迭代后就被抹平了。可通过计算流体内部的微小旋转，然后人为施加一个与涡方向一致的外力，补充涡度（借鉴的网格流体）
