...
注意需要选取合适的阈值以使解算收敛。求出 $p^{(n+1)}$ 之后，再求出 $\nabla p^{(n+1)}$ ，最后使用式 (4) 来更新 $u^{(n+1)}$ . $u^{(n)}_{i+1/2,j}$ 、 $u^{(n)}_{i-1/2,j}$ 、 $u^{(n)}_{i,j+1/2}$ 、 $u^{(n)}_{i,j+\frac{1}{2}}$ 即前一步得到的 $u_B$ .

GS迭代本身不适合GPU并行，但由于网格的高度对称和稠密性，可采用一种“红黑”更新方式：把每个cell按照 $(i+j+k)%2$ 分成两组，先并行更新“红”，再并行更新“黑”，收敛速度约为普通Jacobi迭代的1.5-2倍。除此之外，还有PCG、MGPCG等面向流体求解的高精度求解器。
