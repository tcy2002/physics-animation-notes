...
这篇刚体笔记的求解部分与2005年的书籍Stable, Robust, and Versatile Multibody Dynamics Animation方法基本一致，这是很多游戏物理引擎的基础，在实时性和稳定性上有很好的取舍。
...
- 雅可比迭代、Gauss-Seidel迭代、PGS：参考[MathUtils.md](./MathUtils.md)
...
把$\Delta u$代入到原式子中可以得到：$J_{ab} M^{-1}J_{ab}^T\lambda=-J_{ab}u$，这里$\Delta t$被吸收进$\lambda$里了，由于直接求解会有jitter（就是表现为刚体上下抖动，上一帧有碰撞下一帧没碰撞，反复横跳），所以在右侧加上一个bias项：
$$J_{ab} M^{-1}J_{ab}^T\lambda=-J_{ab}u+b$$
这个式子就是经典的基于冲量（速度）的碰撞约束求解公式。实际计算时bias项是一个与误差修正有关的项，马上会提到。求得$\lambda=\frac{-J_{ab}u+b}{J_{ab} M^{-1}J_{ab}^T}$之后，根据上面求出$J_{ab}^T\lambda$即可得到约束冲量。由于$M^{-1}$是一个不变的质量属性矩阵，所以可以事先计算得到，不需要每次都求逆。
...
- 陀螺问题
  ...
  可能会有疑问：为什么似乎前面的推导过程没体现牛顿-欧拉方程里的进动项$\omega\times I\omega$？因为产生进动项的本质原因是惯性张量$I$在随时间变化，所以对角动量取微分$d(I\omega)$时展开产生了进动项，而上面求的是瞬时冲量，目的是在一瞬间内修正速度，因此将$I$视为常量。也正因为如此，游戏引擎一般无法精确模拟陀螺现象，当角速度非常大时，求解结果或有比较大的误差。
...
- 与基于力的约束的关系
  ...
  前者从单个物体的位置的视角出发，求解满足当前约束条件的约束力，然后通过积分方法来更新位置和速度；后者从两个物体的相对速度视角出发，求解满足当前约束所需的校正冲量。
  ...
  式(d)中的$\frac{\partial \dot C}{\partial q}\cdot \dot q$在前面的推导过程中被忽略了。另外，式(b)中$J_{ab}u$是需要校正的相对速度，式(a)中$JWQ$是外力作用下约束方向上的速度变化量，本质是类似的。
...
为什么这里的$J_{ab}$维度是$3\times 12$而不是和前面一样的$1\times 12$？因为这里的约束有三个自由度，分开写的形式为$e_i \cdot (v_b+\omega_b\times r_b-v_a-\omega_a\times r_a)=0$，$e_i$是轴向单位矢量$e_x,e_y,e_z$

### 距离约束（Distance Joint）

距离约束的条件为：两个刚体上锚点之间的距离不超过指定值，或者始终等于指定值。如果是前者，相当于将锚点用固定长度的绳子连接起来，约束公式与球铰约束相同，但不同的是，“不超过”条件带来了不等式约束和LCP问题，处理方式也很简单，只需要判断距离是否超过指定值即可，超过则需要进行约束，误差修正项为超过的距离，与碰撞约束的Penetration类似。如果是后者，则按照与前面球铰约束类似的方式处理等式约束即可，误差修正项为偏离指定距离的长度。
...
Ragdoll是物理角色动画的一种，用于实时地、符合物理规律地模拟一个角色在失去控制之后（通常是死亡倒下）的角色运动，角色完全基于物理引擎实现，一般是刚体+关节约束，也有可能涉及软体。若需要主动驱动的Ragdoll，可以采用类似机器人控制的方法比如PID，或者与关键帧动画混合使用，比如下半身为关键帧动画，上半身为Ragdoll，辅以一定的关节驱动和IK等。加上蒙皮之后，就可以由真实的布娃娃效果，比如人类一败涂地。

### 关节驱动

角色关节驱动通常的实现方式为：设置一个目标角度或目标角速度，控制关节朝向预期的目标转动。对上面基于冲量（速度）的方法而言，这是很直观和容易实现的，设置目标角速度即增加一个绕旋转轴$n$的约束$(\omega_b - \omega_a) \cdot n = \omega_{target}$，设置目标角度即增加一个类似位置约束的角度约束，误差修正项为当前角度到目标角度的差值。除了对关节施加约束外，还可直接对连接的刚体施加大小相等，方向相反的扭矩，由于扭矩不是点作用而是自由矢量，所以与关节无关。

## 其他形式和方法

### PBD based Rigid

### Featherstone/Ahmed A. Shabana's Rigidbody Dynamics
